...
def getData(symbol):
    ...
    querystr = "select * from masterdata where " + \
                "time_gathered >= '2017-02-14 08:00:00' " + \
...
def cleanCursor(cursor):
    ...
    for entry in cursor:
        newentry = []
        indicies = range(len(entry))

        for k in indicies:
            if k == 2:
                newentry = newentry + [str(entry[k])]
            else:
                item = entry[k]
                newitem =''
                for letter in item:
...
def createTypedTable():
    querystr  = "select * from masterdata where time_gathered >= '2017-01-01 13:00:00' ;"
    query = (querystr)
    cursor = cnx.cursor()
    cursor.execute(query)
    for item in cleanCursor(cursor):
        querystr = "insert into masterdatab values " + str(tuple(item))
        count = 0
